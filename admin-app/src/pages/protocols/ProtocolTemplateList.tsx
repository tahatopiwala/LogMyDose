...
export default function ProtocolTemplateList() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => protocolsApi.getTemplates({
      page,
      limit: 20,
    ...
    <div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-red-500">Error loading templates</div>
        ) : (
          <>
                  ...
                  <tr key={template.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        template.difficultyLevel === 'beginner' ? 'bg-green-100 text-green-800' :
                        template.difficultyLevel === 'intermediate' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                      <Link to={`/protocols/${template.id}/edit`} className="text-primary-600 hover:text-primary-900">Edit</Link>
                      <button
