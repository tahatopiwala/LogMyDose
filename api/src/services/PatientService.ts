...
import { IPatientRepository, PatientWithClinic, UpdatePatientInput } from '../interfaces/repositories/IPatientRepository.js';
import { ITenantRepository } from '../interfaces/repositories/ITenantRepository.js';
import { IProtocolRepository, ProtocolWithDetails } from '../interfaces/repositories/IProtocolRepository.js';
...
export class PatientService implements IPatientService {
  ...
  async updateProfile(patientId: string, data: UpdatePatientInput): Promise<Patient> {
    return this.patientRepository.update(patientId, data);
  }

  async linkToClinic(patientId: string, inviteCode: string): Promise<PatientWithClinic> {
    const invitation = await this.tenantRepository.findInvitationByCode(inviteCode);
    ...
    if (invitation.status !== 'pending') {
      throw new AppError(400, 'This invitation has already been used or expired', 'INVITATION_USED');
    }

    if (new Date() > invitation.expiresAt) {
      throw new AppError(400, 'This invitation has expired', 'INVITATION_EXPIRED');
    }

    await this.patientRepository.linkToClinic(patientId, invitation.clinicId, 'view_only');
  ...
  async getDoses(patientId: string, query: PatientDosesQuery): Promise<PaginatedResponse<Dose>> {
