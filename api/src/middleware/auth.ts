...
export function requireProvider(
  ...
  const allowedRoles: TokenPayload["role"][] = [
    "provider",
    "clinic_admin",
    "super_admin",
  ];
  ...
  if (!allowedRoles.includes(req.user.role)) {
    return next(
      new AppError(403, "Provider access required", "PROVIDER_REQUIRED"),
    );
  }
...
export function requireSuperAdmin(
  ...
  if (req.user.role !== "super_admin") {
    return next(
      new AppError(403, "Super admin access required", "SUPER_ADMIN_REQUIRED"),
    );
  }
